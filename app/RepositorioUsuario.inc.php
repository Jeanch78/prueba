    ...
    public static function insertar_usuario($conexion, $usuario){
        ...
        if (isset($conexion)) {
            try {
                ...
                $sentencia->bindValue(':clave', $usuario->obtener_password(), PDO::PARAM_STR);
    ...
    public static function nombre_existe($conexion,$nombre){
        ...
        if (isset($conexion)) {
            try {
                ...
                $sentencia=$conexion->prepare($sql);
                $sentencia ->bindValue(":nombre",$nombre, PDO::PARAM_STR);//bindValue
                $sentencia->execute();
                $resultado =$sentencia ->fetchAll();
                if (count($resultado)) {
                    $nombre_existe=true;
    ...
    public static function email_existe($conexion,$email){
        ...
        if (isset($conexion)) {
            try {
                ...
                $sentencia=$conexion->prepare($sql);
                $sentencia ->bindValue(":email",$email, PDO::PARAM_STR);//bindValue
                $sentencia->execute();
                $resultado =$sentencia ->fetchAll();
                if (count($resultado)) {
                    $email_existe=true;
